  ...
  get(restaurantsRef).then((snapshot) => {
    snapshot.forEach((childSnapshot) => {
      ...
      const restaurantMarker = new google.maps.Marker({
        ...
        map,
        icon: "/public/ASSETS/map-markers/restaurant.png",
      });
  ...
  get(driversRef).then((snapshot) => {
    snapshot.forEach((childSnapshot) => {
      ...
      const driverMarker = new google.maps.Marker({
        ...
        map,
        icon: "/public/ASSETS/map-markers/driver.png",
      });
...
function showBestDriver(bD) {
  const driverName = bD.name;
  const driverDuration = bD.duration;
  const driverDistance = bD.distance;
  const driverCost = bD.cost;
  const text = "Name: " + driverName + " Duration: " + driverDuration + " minutes Distance: " + driverDistance + " miles Cost: $" + driverCost;
  document.getElementById('driverDetail').innerHTML = text;
}

function showRouteForBestDriver() {
  bestDriver = result.reduce((prev, current) => (prev.cost < current.cost) ? prev : current);
  showBestDriver(bestDriver);
  ...
  geocoder.geocode({ 'address': address }, function (results, status) {
    if (status == 'OK') {
      ...
      deliveryMarker = new google.maps.Marker({
        ...
        map,
        icon: "/public/ASSETS/map-markers/delivery.png",
      });
    }
  });
}
...
function getTotalItems() {
  ...
  for (let i = 0; i < tableBody.rows.length; i++) {
    const quantityCell = tableBody.rows[i].cells[3];
...
function getOrderDetails() {
  ...
  document.getElementById('orderDetail').innerHTML = "Total $:" + sum.toFixed(2);
}
...
function addItem(label, price, quantity) {
  ...
  addButton.addEventListener('click', () => {
    ...
    if (quantity > 0) {
      updateCart(label, price, quantity);
    }
  });
...
function updateCart(label, price, quantity) {
  // Get the table body element
  const tableBody = document.getElementById('cart-items');
